...
module 0xABCD::simple {
    ...
    fun init_module(publisher: &signer) {
        ...
        move_to<FlagAggV2>(
            publisher,
            FlagAggV2 { flag: aggregator_v2::create_aggregator(1) }
        );
    }
    ...
    public entry fun modify_destination_flag_agg_v2(_owner: &signer, destination: address, increment: bool) acquires FlagAggV2 {
        assert!(exists<FlagAggV2>(destination), error::invalid_argument(ECOUNTER_RESOURCE_NOT_PRESENT));
        let flag = borrow_global_mut<FlagAggV2>(destination);
        if (increment) {
            aggregator_v2::try_add(&mut flag.flag, 1);
        } else {
            aggregator_v2::try_sub(&mut flag.flag, 1);
        }
    }
