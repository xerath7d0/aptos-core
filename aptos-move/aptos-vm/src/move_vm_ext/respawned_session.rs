        ...
        match self.change_set.delayed_field_change_set().get(id) {
            ...
            Some(Apply(apply)) => {
                let base_value = match apply.get_apply_base_id(id) {
                    ApplyBase::Previous(base_id) => self.base.get_delayed_field_value(&base_id)?,
                    // For Current, call on self to include current change!
                    ApplyBase::Current(base_id) => self.get_delayed_field_value(&base_id)?,
                };
                Ok(apply.apply_to_base(base_value)?)
            },
            None => self.base.get_delayed_field_value(id),
        }
    }
        ...
        match self.change_set.delayed_field_change_set().get(id) {
            ...
            Some(Apply(DelayedApplyChange::AggregatorDelta { delta: change_delta })) => {
                let merged = &DeltaWithMax::create_merged_delta(
                    ...
                    change_delta)?;
                self.base.delayed_field_try_add_delta_outcome(
                    id,
                    &merged.get_update(),
                    delta,
                    max_value)
            ...
            None => self.base.delayed_field_try_add_delta_outcome(id, base_delta, delta, max_value)
        }
    }
